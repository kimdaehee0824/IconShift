...
macOS has no built-in way to give an app two Finder/Dock icons — one for Light appearance and one for Dark. EasyIcon adds exactly that: assign a light icon and a dark icon to any app, and it re-applies the right one whenever the system appearance changes.
...
- 🧭 **Menu-bar first, or invisible** — run with a menu-bar icon, or hide it entirely and keep EasyIcon alive purely in the background.
...
```bash
tuist generate        # generates EasyIcon.xcworkspace and opens it in Xcode
```

Pick the **EasyIcon** scheme and press **Cmd+R**. This runs the real signed `.app`
...
> once, then set the identity to **EasyIcon Self-Signed** in
...
scripts/build.sh               # assembles dist/EasyIcon.app
open dist/EasyIcon.app
```
...
Changing another app's icon requires macOS's **App Management** permission (this is a hard OS requirement, not an EasyIcon setting). On first use:

1. Add an app and assign it an icon.
2. macOS shows a prompt — click **Allow**. If you miss it, open
   **System Settings › Privacy & Security › App Management** and enable **EasyIcon**.
3. Hit **Apply Now** (or just toggle appearance).

EasyIcon shows a banner and an "Open Settings" button whenever a write is blocked.
...
> **Dock note:** the Finder icon updates immediately. An app that is *already running* keeps its old Dock tile until you quit and relaunch it — this is how the macOS Dock works, not an EasyIcon bug.

Icons and config live in `~/Library/Application Support/EasyIcon/` (config.json, icons/, easyicon.log).
...
- **Applying icons** — `NSWorkspace.setIcon(_:forFile:)`. On apps registered in
  ...
  **App Management**, which is why EasyIcon must be code-signed and granted permission.
- **Crash/partial-write safety** — a failed `setIcon` can leave the FinderInfo
  custom-icon bit set without icon data, which shows a blank/generic icon. EasyIcon
...
[MIT](LICENSE) © 2026 EasyIcon Contributors
